	...
	return (
		<ProtectedRoute>
			<main className="max-w-4xl mx-auto px-4 py-10 space-y-8">
				<section className="rounded-xl border border-[var(--border)] bg-[var(--muted)] p-6 shadow-sm">
					<h1 className="text-3xl font-semibold text-[var(--foreground)] mb-4">{t("User Panel")}</h1>
					<p className="text-[var(--muted-font-color)]">{t("Review your account information and manage security preferences.")}</p>
				</section>

				<section className="rounded-xl border border-[var(--border)] bg-white dark:bg-[var(--muted)] p-6 shadow-sm">
					<h2 className="text-xl font-semibold text-[var(--foreground)] mb-4">{t("Account overview")}</h2>
					<dl className="grid grid-cols-1 md:grid-cols-2 gap-4">
						<div>
							<dt className="text-sm text-[var(--muted-font-color)]">{t("Email")}</dt>
							<dd className="text-lg text-[var(--foreground)] font-medium break-all">{user?.email}</dd>
						</div>
						<div>
							<dt className="text-sm text-[var(--muted-font-color)]">{t("User ID")}</dt>
							<dd className="text-lg text-[var(--foreground)] font-mono break-all">{user?.id}</dd>
						</div>
						<div>
				...
				<section className="rounded-xl border border-[var(--border)] bg-white dark:bg-[var(--muted)] p-6 shadow-sm">
					<h2 className="text-xl font-semibold text-[var(--foreground)] mb-4">{t("Profile details")}</h2>
					<form onSubmit={handleProfileUpdate} className="space-y-4">
						<label className="flex flex-col gap-2">
							...
							<input
								...
								className="rounded-md border border-[var(--border)] px-3 py-2 bg-transparent text-[var(--foreground)]"
								placeholder={t("Enter a public display name")}
							/>
						</label>
						<button
							type="submit"
							className="btn-primary px-4 py-2 font-semibold disabled:opacity-60"
				...
				<section className="rounded-xl border border-[var(--border)] bg-white dark:bg-[var(--muted)] p-6 shadow-sm">
					<h2 className="text-xl font-semibold text-[var(--foreground)] mb-4">{t("Change password")}</h2>
